...
Codex 个人技能插件 marketplace，包含一个名为 `worktools` 的插件包，内含 7 个工作流技能。
...
```
plugins/worktools/
├── .Codex-plugin/plugin.json   # 插件元数据（name, version）
└── skills/
    ...
    └── optimize-Codex-md/      # AGENTS.md 拆分优化（SKILL.md）
```

Marketplace 注册在 `.Codex-plugin/marketplace.json`，插件定义在 `plugins/worktools/.Codex-plugin/plugin.json`。
...
插件版本号在 `plugins/worktools/.Codex-plugin/plugin.json` 的 `version` 字段。修改技能后需要 bump 版本号。
